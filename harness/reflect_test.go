...
func TestProcessBookingSource(t *testing.T) {
	revel.Init("prod", "github.com/Gazillion-Platform-Team/revel/samples/booking", "")
	sourceInfo, err := ProcessSource([]string{revel.AppPath})
	if err != nil {
		t.Fatal("Failed to process booking source with error:", err)
	}

	CONTROLLER_PKG := "github.com/Gazillion-Platform-Team/revel/samples/booking/app/controllers"
...
func BenchmarkProcessBookingSource(b *testing.B) {
	revel.Init("", "github.com/Gazillion-Platform-Team/revel/samples/booking", "")
